...
The showcase uses left and right arrows to move between feature scenes for the full feedback stack, sound controls, particles, and camera/screen adapter behavior.

## LOVE Sound, Camera + Screen Adapter

`feel.love` is an optional LOVE2D adapter for sound, particles, camera, and screen feedback. It plays registered audio cues, updates and draws registered particle systems, handles adapter `emit` events, keeps the state, and gives you small draw helpers.
...
Supported adapter events are `sound.play`, `sound.stop`, `sound.pause`, `sound.resume`, `sound.volume`, `sound.pitch`, `sound.pan`, `particle.emit`, `particle.start`, `particle.stop`, `particle.reset`, `particle.move`, `camera.shake`, `camera.zoom`, `camera.move`, `camera.reset`, `screen.flash`, `screen.fade`, and `screen.clear`.
...
LOVE-first helpers should build on those primitives instead of expanding the core into one component per effect. `sound`, `particle`, `camera`, and `screen` are adapter-backed families today; good future helper families include `text`, `sprite`, `time`, `spring`, and `shake`.
